...
insert into auth.users (id, email) values
  ...
  ('d0000000-0000-4000-8000-000000000105', 'frontend-cashier@test.invalid');
...
insert into public.profiles (id, full_name) values
  ...
  ('d0000000-0000-4000-8000-000000000105', 'Frontend Cashier');
...
insert into public.organization_memberships (organization_id, user_id, role) values
  ...
  ('d0000000-0000-4000-8000-000000000001', 'd0000000-0000-4000-8000-000000000105', 'cashier');
...
set local request.jwt.claim.sub = 'd0000000-0000-4000-8000-000000000105';
select is((select count(*) from public.customer_ledger_entries), 2::bigint, 'Cashier can view the customer Ledger');
select is((select count(*) from public.collectibles), 1::bigint, 'Cashier can view Collectibles');
select is((select count(*) from public.payments), 1::bigint, 'Cashier can view Payments');
select throws_ok(
  $$select public.record_payment(
    'd0000000-0000-4000-8000-000000000001', 'd0000000-0000-4000-8000-000000000501', '2026-09-13', 1, 'gcash',
    'd0000000-0000-4000-8000-000000000702', null, null, 'missing reference test'
  )$$,
  '22023', null, 'electronic Payment requires a reference'
);
select lives_ok(
  $$select public.record_payment(
    'd0000000-0000-4000-8000-000000000001', 'd0000000-0000-4000-8000-000000000501', '2026-09-13', 2500, 'bank',
    'd0000000-0000-4000-8000-000000000703', null, 'BANK-FRONT-1', 'cashier verified'
  )$$,
  'Cashier can perform an allowed Payment workflow'
);
...
select throws_ok(
  $$select public.transfer_warehouse_to_salesman(
    'd0000000-0000-4000-8000-000000000001', 'd0000000-0000-4000-8000-000000000103', '2026-09-13',
    jsonb_build_array(jsonb_build_object('inventory_lot_id', (select id from public.inventory_lots where organization_id = 'd0000000-0000-4000-8000-000000000001'), 'quantity_kg', 1)),
    'd0000000-0000-4000-8000-000000000704'
  )$$,
  '42501', null, 'Cashier cannot mutate inventory through transfer RPCs'
);
select results_eq(
  $$update public.plants set name = 'Cashier Unauthorized' where id = 'd0000000-0000-4000-8000-000000000201' returning 1$$,
  $$select 1 where false$$,
  'Cashier cannot edit Plant configuration'
);
select is((select count(*) from public.payroll_entries), 0::bigint, 'Cashier cannot access payroll data');
select is((select notes from public.payments where client_request_id = 'd0000000-0000-4000-8000-000000000703'), 'cashier verified', 'Cashier Payment notes are retained');
select is((select count(*) from public.inventory_movements where movement_type = 'sale'), 0::bigint, 'Cashier cannot view inventory movements');
