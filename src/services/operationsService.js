...
export async function loadOperationsSummary(organizationId) {
  ...
  const [warehouse, salesman, trips, sales, payments, expenses, dcrs, plantSales, productSales] = await Promise.all([
    ...
    client.from("payments").select("payment_date, amount, method, salesman_user_id, created_at").eq("organization_id", organizationId).is("voided_at", null),
    client.from("expenses").select("id, expense_date, category, amount, payment_source, description, approval_status, salesman_user_id, created_by, created_at").eq("organization_id", organizationId),
