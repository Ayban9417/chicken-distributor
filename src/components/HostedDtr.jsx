...
const freshEntry = (userId) => ({ userId, date: today(), timeIn: "08:00", timeOut: "17:00", breakMinutes: "60", correctionReason: "" });
...
export function HostedDtr({ organizationId, userId, role }) {
  ...
  async function submit() {
    ...
    if (editing.id && !editing.correctionReason?.trim()) return setStatus({ busy: false, error: "Enter a correction reason for the audit trail.", notice: "" });
    setStatus({ busy: true, error: "", notice: "" });
    try {
      await saveTimeEntry(organizationId, editing);
      setEditing(null);
      setStatus({ busy: false, error: "", notice: editing.id ? "DTR correction saved and audited." : "DTR entry saved." });
  ...
  return <div>
    <SectionHeader title="DTR" eyebrow="Attendance records" action={<div className="flex flex-wrap gap-2"><Button variant="secondary" aria-label="Refresh DTR" title="Refresh DTR" onClick={entries.refresh}><RefreshCw size={17} /></Button>{admin && <Button onClick={startNew}><Plus size={17} />New Entry</Button>}</div>} />
    ...
    {editing && admin && <section className="report-section"><h2 className="mb-4 text-lg font-bold">{editing.id ? "Correct DTR Entry" : "New DTR Entry"}</h2><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Field label="Employee"><select className={inputClass()} value={editing.userId} onChange={(event) => setEditing({ ...editing, userId: event.target.value })}><option value="">Select Employee</option>{(people.data || []).filter((person) => person.active || person.user_id === editing.userId).map((person) => <option key={person.user_id} value={person.user_id}>{person.full_name}</option>)}</select></Field><Field label="Date"><input className={inputClass()} type="date" max={today()} value={editing.date} onInput={(event) => setEditing({ ...editing, date: event.target.value })} /></Field><Field label="Time In"><input className={inputClass()} type="time" step="60" value={editing.timeIn} onInput={(event) => setEditing({ ...editing, timeIn: event.target.value })} /></Field><Field label="Time Out (Optional)"><input className={inputClass()} type="time" step="60" value={editing.timeOut} onInput={(event) => setEditing({ ...editing, timeOut: event.target.value })} /></Field><Field label="Break (Minutes)"><input className={inputClass()} type="number" min="0" step="1" value={editing.breakMinutes} onChange={(event) => setEditing({ ...editing, breakMinutes: event.target.value })} /></Field></div>{editing.id && <Field label="Correction Reason"><textarea className={`${inputClass()} mt-3 min-h-20 py-3`} value={editing.correctionReason || ""} onChange={(event) => setEditing({ ...editing, correctionReason: event.target.value })} /></Field>}<p className="mt-3 text-sm font-semibold text-slate-600">Calculated time: {formatDtrMinutes(calculateDtrMinutes(editing.timeIn, editing.timeOut, editing.breakMinutes))}</p><div className="mt-4 flex flex-wrap gap-2"><Button disabled={status.busy || (editing.id && !editing.correctionReason?.trim())} onClick={submit}><Save size={17} />{status.busy ? "Saving..." : "Save Entry"}</Button><Button variant="ghost" disabled={status.busy} onClick={() => setEditing(null)}>Cancel</Button></div></section>}
    <section className="report-section"><h2 className="mb-4 text-lg font-bold">Attendance Records</h2>{entries.loading && !entries.data ? <p className="py-8 text-center text-slate-500">Loading hosted DTR records...</p> : <ResponsiveTable columns={["Employee", "Date", "Time In", "Time Out", "Break", "Total", "Status", "Actions"]} rows={records.map((row) => [personName(row.user_id), shortDate(row.work_date), shortTime(row.time_in), shortTime(row.time_out) || "-", `${row.break_minutes} min`, formatDtrMinutes(row.total_minutes), <Badge tone={row.time_out ? "green" : "amber"}>{row.time_out ? "Complete" : "Clocked In"}</Badge>, admin ? <Button variant="ghost" onClick={() => { setEditing(timeEntryForm(row)); setStatus({ busy: false, error: "", notice: "" }); }}><Pencil size={16} />Correct</Button> : "-"])} />}</section>
  </div>;
}
