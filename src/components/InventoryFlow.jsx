...
export function Warehouse({ inventoryRows, trips, movements, receivingTransfers, setReceivingTransfers, salesmanTransfers, users, addAudit, pushToast, onOpen }) {
  const rows = useMemo(() => warehouseRows(inventoryRows, receivingTransfers), [inventoryRows, receivingTransfers]);
  const salesmen = activeSalesmen(users);
  const totals = companyStockTotals(inventoryRows, receivingTransfers, salesmanTransfers, movements, salesmen.map((user) => user.id));
  ...
  function confirm() {
    ...
    setReceivingTransfers((items) => [item, ...items]); addAudit(`Created ${receipt}: Warehouse to ${salesmen.find((user) => user.id === item.toSalesmanId)?.name}`, "Warehouse"); pushToast(receipt + " created"); setTransfer(initialTransfer());
  }
  return <><SectionHeader title="WAREHOUSE" eyebrow="PLANTS > WAREHOUSE > SALESMAN INVENTORY" />
    ...
    {transfer.rowId && <section className="report-section"><h2 className="mb-3 text-lg font-extrabold uppercase">Transfer to Salesman</h2><p className="mb-3 font-bold uppercase">{row && stockName(row)} / {row?.plant} / {shortDate(row?.tripDate)}</p>
      ...
      <div className="mt-3 flex gap-2"><Button onClick={confirm}><ClipboardCheck size={17} />Create Receiving Receipt</Button><Button variant="secondary" onClick={() => setTransfer(initialTransfer())}>Cancel</Button></div></section>}
    ...
    <section className="report-section"><h2 className="mb-3 text-lg font-extrabold uppercase">Receiving Receipts</h2><ResponsiveTable columns={["Receipt", "Date", "Salesman", "Plant / Trip", "Product / Code / Class", "KG", "Bags", "Cost/kg", "Notes"]} rows={receivingTransfers.map((item) => [item.receipt, shortDate(item.date), users.find((user) => user.id === item.toSalesmanId)?.name || "-", item.plant + " / " + item.tripCode, productLabel(item.product, item.sizeCode, item.classType, item.sizeCodeLabel, item.classTypeLabel), kg(item.qty), item.bags ?? "Not recorded", currency(item.costPerKg), item.notes || "-"])} /></section>
  </>;
}

export function SalesmanInventory({ inventoryRows, receivingTransfers, salesmanTransfers, setSalesmanTransfers, movements, users, addAudit, pushToast, onOpen, onWarehouse }) {
  const salesmen = activeSalesmen(users); const [salesmanId, setSalesmanId] = useState(salesmen[0]?.id || "");
  const rows = useMemo(() => salesmanInventoryRows(inventoryRows, receivingTransfers, salesmanTransfers, movements, salesmanId), [inventoryRows, receivingTransfers, salesmanTransfers, movements, salesmanId]);
  ...
  function confirm() {
    ...
    if (bags !== null && (!Number.isInteger(bags) || bags < 0)) return setError("Bags must be a whole nonnegative number or left blank.");
    const receipt = nextReceipt("TF", salesmanTransfers); const item = { id: crypto.randomUUID(), receipt, date: demoToday, at: new Date().toISOString(), fromSalesmanId: salesmanId, toSalesmanId: transfer.toSalesmanId,
  ...
  return <><SectionHeader title="INVENTORY" eyebrow="SALESMAN INVENTORY" action={<Field label="Salesman"><select className={inputClass()} value={salesmanId} onChange={(e) => { setSalesmanId(e.target.value); setTransfer(initialTransfer()); }}>{salesmen.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field>} />
    ...
    <section className="report-section"><h2 className="mb-3 text-lg font-extrabold uppercase">Transfer Receipts</h2><ResponsiveTable columns={["Receipt", "Date", "From", "To", "Plant / Trip", "Product / Code / Class", "KG", "Bags", "Notes"]} rows={salesmanTransfers.map((item) => [item.receipt, shortDate(item.date), users.find((user) => user.id === item.fromSalesmanId)?.name || "-", users.find((user) => user.id === item.toSalesmanId)?.name || "-", item.plant + " / " + item.tripCode, productLabel(item.product, item.sizeCode, item.classType, item.sizeCodeLabel, item.classTypeLabel), kg(item.qty), item.bags ?? "Not recorded", item.notes || "-"])} /></section>
  </>;
}
